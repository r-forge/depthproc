depthPersp<-function(X,method = "Projection",plot.method = "rgl",xlim = extendrange(X[,1],f=0.1),ylim = extendrange(X[,2],f=0.1),n=50,
	xlab = "x", ylab = "y",plot.title=paste(method,"depth"),...)
{
	if(dim(X)[2]==2)
	{
			...
			if(plot.method == "rgl") persp3d(axis_x,axis_y,z_surface,color=col,back = "lines",
				xlab = xlab, ylab = ylab,zlab=plot.title)
			
			if(plot.method == "lattice") wireframe(z_surface~xy_surface[,1]+xy_surface[,2],colorkey = TRUE,drape = TRUE,
